...
import ru.chedmitriy.exceptions.CatchArrayOutOfBoundException;
...
public class DynamicArray<E> implements Iterable<E>  {
     ...
     * Параметр
     * */
    private final int capacityUpOnForOnePositionHigher = 1;
    ...
    public <E> E[] resizeContainer() {
        E[] containerBuffer = (E[]) Arrays.copyOf(dynamicArray.objectsIterator,
                dynamicArray.objectsIterator.length
                        + capacityUpOnForOnePositionHigher);
    ...
    public <E> E get(int index) throws CatchArrayOutOfBoundException {
        if (index >= size || index < 0) {
            throw new CatchArrayOutOfBoundException();
        }
