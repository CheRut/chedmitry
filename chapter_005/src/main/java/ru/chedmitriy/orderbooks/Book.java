package ru.chedmitriy.orderbooks;
...
class Book {
    ...
    public Book() {
        io = new InputOutput();
        this.sell = new TreeMap<>(SELL_COMP);
        this.buy = new TreeMap<>(BUY_COMP);
    }
    /**
     * Компаратор сортировки по возрастанию для
     * ордеров на продажу
     */
    private static final Comparator<Float> SELL_COMP = new Comparator<Float>() {
        @Override
     ...
     * на покупку
     */
    private static final Comparator<Float> BUY_COMP = new Comparator<Float>() {
        @Override
        public int compare(Float o1, Float o2) {
    ...
    public void dealChecker(Book book) {
        ...
        int size = buyList.size() >= sellList.size() ? buyList.size()
                : sellList.size();
            ...
            try {
                if (buyList.get(i).getPrice() < sellList.get(i).getPrice()
                    ...
                    io.println(String.format("%s \t\t%s",
                            buyList.get(i).getVolume() + "@" + buyList.get(i).getPrice(),
